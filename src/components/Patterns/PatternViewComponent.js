...
const PatternViewComponent = (props) => {
  ...
  const [isPortraitMode, setIsPortraitMode] = React.useState(
    window.innerWidth < window.innerHeight && window.innerWidth < 960
  );
  const [filterAnchorEl, setFilterAnchorEl] = React.useState(undefined);

  const [initialStateOnPageLoad, setInitialStateOnPageLoad] = React.useState(
    true
  );
  ...
  const [
    isCreatePatternListDialogOpen,
    setCreatePatternListDialogOpen,
  ] = React.useState(false);
  const [
    multipleStakeholdersBeingRendered,
    setMultipleStakeholdersBeingRendered,
  ] = React.useState(false);
  ...
  const getStakeholderInfo = (data) => {
    const stakeholderInfo = data
      .map((level) =>
        level.filter((entity) => entity.description === "Stakeholders")
      )
      .flat()[0]
      .data.map((item) => item);
  ...
  const [stakeholderVisibility, setStakeholderVisibility] = React.useState(
    Array(stakeholderInfo.length).fill(false)
  );
  ...
  const filterData = (data) => {
    return data
      .map((level) => level.filter((entity) => entity.data.length != 0))
  ...
  const filterConcers = (data) => {
    const concerns = data
      .map((level) =>
        level.filter((entity) => entity.description === "Concerns")
      )
      .flat()[0]
        ...
        return {
          ...
          data: entity.data.sort((a, b) => {
            const aIndex = parseInt(
              a.identifier.slice(a.identifier.indexOf("-") + 1)
            );
            const bIndex = parseInt(
              b.identifier.slice(b.identifier.indexOf("-") + 1)
            );
  ...
  const getNumGroupGrid = (level, maxColumns) => {
    const categories = level
      .map((entity) => entity.categories.map((category) => category.data))
      .flat();
    if (
      categories.reduce((count, group) => count + group.length, 0) <=
      maxColumns - (categories.length + 1)
    ) {
      return {
        maxRows: 1,
        maxColumns: Math.max(...categories.map((group) => group.length)),
      };
    }

    let maxGroupColumns =
      maxColumns - (categories.length + 1) - (categories.length - 1);
    while (
      maxColumns - (categories.length + 1) <
      categories.reduce(
        (count, group) => count + Math.min(maxGroupColumns, group.length),
        0
      )
    ) {
      maxGroupColumns--;
    ...
    return {
      maxRows: Math.ceil(
        Math.max(...categories.map((group) => group.length)) / maxGroupColumns
      ),
      maxColumns: maxGroupColumns,
    };
  };
  ...
  const calcGridData = (data, maxColumns) => {
    return data.map((level) => {
      ...
      const numColumnsPerEntity = level.map(
        (entity) =>
          entity.categories.reduce(
            (count, category) =>
              count + Math.min(category.data.length, groupGrid.maxColumns) + 1,
            0
          ) + 1
      );
      return {
        rows: groupGrid.maxRows + 1,
        columns: numColumnsPerEntity.reduce(
  ...
  const calcPositionData = (data, height, width) => {
    ...
    return data.map((level, levelIndex) => {
      ...
      const levelStartY =
        (data
          .slice(0, levelIndex)
          .reduce((count, level) => count + level.rows, 0) /
          numTotalRows) *
        height;
      ...
      return {
        ...
        entities: level.entities.map((entity, entityIndex) => {
          const entityStartX =
            (level.entities
              .slice(0, entityIndex)
              .reduce((count, entity) => count + entity.columns, 0) /
              level.columns) *
            levelWidth;
          ...
          return {
            ...
            categories: entity.categories.map((category, groupIndex) => {
              const groupStartX =
                ((entity.categories
                  .slice(0, groupIndex)
                  .reduce((count, category) => count + category.columns, 0) +
                  groupIndex +
                  1) /
                  entity.columns) *
                entityWidth;
              const groupStartY =
                ((entity.rows - category.rows) / 2 / entity.rows) *
                entityHeight;
              const groupHeight = (category.rows / entity.rows) * entityHeight;
              const groupWidth =
                (category.columns / entity.columns) * entityWidth;
              return {
                categoryName: category.categoryName,
                ...
                data: category.data.map((item, nodeIndex) => {
                    ...
                    nodeStartX =
                      (((category.columns -
                        (category.data.length -
                          (category.rows - 1) * category.columns)) /
                        2 +
                        ((nodeIndex % category.columns) + 0.5)) /
                        category.columns) *
                      groupWidth;
                  } else {
                    nodeStartX =
                  ...
                  const nodeStartY =
                    ((Math.floor(nodeIndex / category.columns) + 0.5) /
                      category.rows) *
                    groupHeight;
                  return {
                    ...item,
                    startX:
                      levelStartX + entityStartX + groupStartX + nodeStartX,
                    startY:
                      levelStartY + entityStartY + groupStartY + nodeStartY,
                  };
                }),
              };
            }),
    ...
    data.forEach((level) => {
      level.forEach((entity) => {
        entity.data.forEach((item) => {
          relations
            .filter((relation) => Object.keys(item).includes(relation.label))
            .forEach((relation) => {
              item[relation.label].forEach((link) => {
                const filteredEntities = data[relation.level].filter(
                  (entity) => entity.description === relation.entityName
                );
                if (filteredEntities.length !== 0) {
                  const related = filteredEntities[0].data.filter(
                    (relatedItem) => relatedItem.identifier === link.identifier
                  )[0];
                  if (related) {
  ...
  const getLinkPositions = (links, positionData) => {
    let linkPositions = [];
    links.forEach((link) => {
      const linkItems = link.map((linkItem) =>
        positionData
          .map((level) =>
            level.entities.map((entity) =>
              entity.categories.map((category) =>
                category.data.filter((item) => item.identifier === linkItem)
              )
            )
          )
          .flat()
      ...
      if (linkItems[0].length + linkItems[1].length === 2) {
        linkPositions.push([
          {
            identifier: link[0],
            startX: linkItems[0][0].startX,
            startY: linkItems[0][0].startY,
          },
          {
            identifier: link[1],
            startX: linkItems[1][0].startX,
            startY: linkItems[1][0].startY,
          },
        ]);
      }
    });
  ...
  const getPath = (links, direction, identifier) => {
    ...
    const relative = links
      .filter(
        (link) => (direction === "from" ? link[0] : link[1]) === identifier
      )
      .map((link) => (direction === "from" ? link[1] : link[0]));
  ...
  const getAllLinkPaths = () => {
    ...
    selectedStakeholders.map((val) => {
      if (val !== null) {
        array = array.concat(
          getPath(links, "from", val.identifier).concat(
            getPath(links, "to", val.identifier)
          )
        );
      }
    });
  ...
  const getSelectedData = (data) => {
    const selectedItem =
      history.location.pathname.split("/").filter((word) => word !== "")
        .length > 1
  ...
  const relations = filteredData
    .map((level, index) =>
      level.map((entity) => ({
        entityName: entity.description,
        label:
          entity.description.charAt(0).toLowerCase() +
          entity.description.slice(1).replace(/\s/g, ""),
        level: index,
      }))
    )
    .flat();
  ...
  const handleNodeEnter = (identifier) => {
    ...
    setOnPath(
      getPath(links, "from", identifier)
        .concat(getPath(links, "to", identifier))
        .slice(1)
    );
  };
  ...
  const handleSelectStakeholder = (item, index) => {
    if (
      stakeholderVisibility.filter((value) => value === true).length > 2 &&
  ...
  const renderStakeholderChips = () => {
    return stakeholderInfo.map((item, index) => {
      return (
        <Chip
          color={stakeholderVisibility[index] === true ? "primary" : "default"}
          className={classes.stakeholderChips}
          avatar={<Avatar className={classes.avatar}>{item.identifier}</Avatar>}
          label={item.name}
          onClick={() => handleSelectStakeholder(item, index)}
        />
      );
    });
  };
        ...
        if (val !== null) {
          return (
            <Chip
              ...
              avatar={
                <Avatar className={classes.avatar}>{val.identifier}</Avatar>
              }
              label={val.name}
              onDelete={() => removeSelectedStakeholder(val.identifier)}
            />
          );
        }
      });
  ...
  if (
    history.location.pathname.split("/").filter((word) => word !== "").length >
    1
  ) {
    return (
      <div>
        ...
        <SinglePatternViewComponent
          ...
          data={
            filteredData
              .map((level) =>
                level.map((entity) =>
                  entity.data.filter((item) => item.identifier === selectedItem)
                )
              )
              .flat()
              .flat()[0]
  ...
  return (
    <div className={classes.root}>
      <Helmet>
        ...
        <meta
          name="description"
          content="Large Scale Agile development Patterns"
        />
          ...
          >
            ...
            <Grid item xs={12}>
              <Typography variant="h4" component="p">
                Rotate your screen to view visualizations.
              </Typography>
            </Grid>
          </Grid>
        </div>
        ...
        <Paper elevation={6} className={classes.paper}>
          <div className={classes.paperView}>
            {initialStateOnPageLoad ? (
              <Typography variant="h4">
                Please select max. 3 stakeholders to view the Pattern Graph
              </Typography>
            ) : (
            ...
            >
              {filteredData[filterIndices.level][filterIndices.entity].data
                .concat()
                .sort((a, b) =>
                ...
                .map((item, index) => (
                  <MenuItem
                    ...
                    style={{
                      color:
                        selected &&
                        selected.identifier === item.identifier &&
                        "grey",
                    }}
                    onClick={() => handleFilterChange(item)}
                  >{`${item.identifier} - ${item.name}`}</MenuItem>
                ))}
            </Menu>
          </div>
          <div className={classes.chipsView}>
            {selected && !multipleStakeholdersBeingRendered && (
              <Chip
                className={classes.chip}
                avatar={
                ...
                label={selected.name}
                onDelete={resetToInitialPageState}
              />
            )}
          </div>
          <div className={classes.chipsView}>
          ...
          {ItemsService.isLoggedIn() && ItemsService.isAdminAndVerified() ? (
            <Fab
              ...
              onClick={handleCreatePatternListDialogOpen}
            >
              <AddIcon className={classes.button} />
              Create
            </Fab>
          ) : null}
          <CreatePatternsListDialog
            open={isCreatePatternListDialogOpen}
            data={filteredData}
            handleClose={handleCreatePatternListDialogClose}
          />
          <Snackbar
            ...
            message={
              <span id="message-id">
                Cannot select more than 3 Stakeholders!
              </span>
            }
          />
        </Paper>
