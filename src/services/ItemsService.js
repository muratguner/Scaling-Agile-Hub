...
export default class ItemsService {
  ...
  static isLoggedIn() {
    if (
      ...
      Date.now() < JSON.parse(localStorage.getItem("authentication")).expires
    ) {
      return true;
    }
    return false;
  ...
  static isAdminAndVerified() {
    if (
      ...
      JSON.parse(localStorage.getItem("userInfo")).isVerified === true
    ) {
      return true;
    }
    return false;
  ...
  static getFrameworks() {
    return FetchService.sendPost(WORKSPACE_URL, {
      expression: this.getFrameworksQuery(),
    });
  }

  static getFramework(id) {
    return FetchService.sendPost(WORKSPACE_URL, {
      expression: this.getFrameworksQuery() + '.where(id = "' + id + '" )',
    });
  }

  static getFrameworkVersions() {
    return FetchService.sendPost(WORKSPACE_URL, {
      expression: this.getFrameworkVersionsQuery(),
    });
  }
  ...
  static getFrameworkVersion(id) {
    return FetchService.sendPost(WORKSPACE_URL, {
      expression:
        this.getFrameworkVersionsQuery() + '.where(id = "' + id + '" )',
    });
  }
  ...
  static getPatternElement(entity, id) {
    ...
    return FetchService.sendPost(PATTERNS_MXL_URL, {
      expression:
        entities[entity.replace(/\s/g, "")]() + '.where(id = "' + id + '" )',
    });
  }
    ...
    return [
      ...
      [
        { description: "Principles", query: this.getPrinciplesQuery },
        {
          ...
          query: this.getCoordinationPatternsQuery,
        },
        {
          description: "Methodology Patterns",
          query: this.getMethodologyPatternsQuery,
        },
      ],
      [
        {
          description: "Visualization Patterns",
          query: this.getVisualizationPatternsQuery,
        },
      ],
    ].map((level) => {
      return level.map((entity) => {
        return {
          ...
          promise: FetchService.sendPost(PATTERNS_MXL_URL, {
            expression: entity.query(),
          }),
        };
      });
    });
    ...
    return [
      {
        ...
        element: "Foundational Method",
        relation: "Foundational Methods",
      },
      {
        description: "Agile Activities",
        element: "Foundational Activity",
        relation: "Foundational Activities",
      },
      {
        description: "Agile Principles",
        element: "Foundational Principle",
        relation: "Foundational Principles",
      },
      {
        description: "Agile Artifacts",
        element: "Foundational Artifact",
        relation: "Foundational Artifacts",
      },
    ].map(({ description, element, relation }) => ({
      description: description,
      promise: FetchService.sendPost(WORKSPACE_URL, {
        expression: `find('${element}').select({id, name, frameworks: get Framework whereis '${relation}'.select({id, name})})`,
      }),
    }));
  }
  ...
  static getCoordinationPatternsQuery() {
    return (
      "find('Coordination Pattern').select({" +
      "id: id," +
      "identifier: identifier," +
      "name: Name," +
  ...
  static getMethodologyPatternsQuery() {
    return (
      "find('Methodology Pattern').select({" +
      "id: id," +
      "identifier: identifier," +
      "name: Name," +
  ...
  static getVisualizationPatternsQuery() {
    return (
      "find('Visualization Pattern').select({" +
      "id: id," +
      "identifier: identifier," +
      "name: Name," +
      "knownUses: 'known uses'," +
  ...
  static getToken(username, password) {
    return FetchService.sendPost("https://sah.sebis.in.tum.de/api/v1/jwt", {
      username: username,
      password: password,
    });
  }
  ...
  static getUserInfo(userId, token) {
    return FetchService.sendGet(
      `https://sah.sebis.in.tum.de/api/v1/users/${userId}`,
      { Authorization: `Bearer ${token}` }
    );
  }

  static getNewUser(name, email, password) {
    return FetchService.sendPost("https://sah.sebis.in.tum.de/api/v1/users", {
      name: name,
      email: email,
      password: password,
    });
  ...
  static createPattern(id, name, attributes) {
    return FetchService.sendPost(
      "https://sah.sebis.in.tum.de/api/v1/entityTypes/" + id + "/entities",
      {
        name: name,
  ...
  static getClapCount() {
    return {
      promise: FetchService.sendPost(RATINGS_MXL_URL, {
        expression: this.getRatingsQuery(),
      }),
    };
  }
  ...
  static getPicture(id) {
    return FetchService.imageGet(
      "https://sah.sebis.in.tum.de/api/v1/users/" + id + "/picture",
      { authorization: this.getBearerToken() }
    );
  }
  ...
  static uploadPatternsEntityImage(payload) {
    return FetchService.uploadFile(
      "https://sah.sebis.in.tum.de/api/v1/files/new",
      payload,
      { Accept: "application/json", authorization: this.getBearerToken() }
    );
  }
