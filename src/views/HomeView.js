...
const HomeView = props => {
	...
	React.useEffect(() => {
		...
		const allCommentsData = ItemsService.getComments();
		let dataSet = []
		allUnresolvedData.forEach((level, indexLevel) => {
			...
			level.forEach(entity => {
				entity.promise.then(data => {
					...
					if (dataSet.reduce((sum, level) => sum + level.length, 0) === numOfEntities) {
		...
		allCommentsData.promise.then(data => {
	...
	return (
		<div className={classes.root}>
			...
			<HomeViewComponent allData={allData} allComments={allComments} allRatings={allRatings}/>
			<Footer />
		</div>
	)
}
